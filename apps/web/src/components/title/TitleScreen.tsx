...
const MENU_ITEMS: MenuItem[] = [
  ...
  { id: 'settings', label: 'Configurações', available: false },
  { id: 'credits', label: 'Créditos', available: false },
];
...
export function TitleScreen({ ready, onStart }: { ready: boolean; onStart: () => void }) {
  ...
  const activate = useCallback(
    (index: number) => {
      if (leaving || !MENU_ITEMS[index].available) return;
      if (MENU_ITEMS[index].id === 'play') setLeaving(true);
    },
    [leaving]
  );
